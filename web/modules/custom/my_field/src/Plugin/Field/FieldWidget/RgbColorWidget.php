...
class RgbColorWidget extends WidgetBase {
  ...
  /**
   * Massages an array of RGB color values into hexadecimal color codes.
   *
   * @param array $values
   *   An array of color values, each containing 'red', 'green' and 'blue' keys.
   * @param array $form
   *   The form structure.
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    // Loop through each color value in the array.
    foreach ($values as &$value) {
      // Convert 'red', 'green', and 'blue' values to integers.
      $red = (int) $value['red'];
      $green = (int) $value['green'];
      $blue = (int) $value['blue'];

      // Convert RGB values to hexadecimal color code and store it in 'hexcode'.
  ...
  public function convertToRgb($hex_value) {
    [$r, $g, $b] = sscanf($hex_value, "#%02x%02x%02x");
    return [$r, $g, $b];
  }
